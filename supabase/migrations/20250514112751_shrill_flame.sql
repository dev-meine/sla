/*
  ...
  1. Storage Configuration
    - Creates public 'images' bucket if it doesn't exist
    - Enables RLS on storage tables
    - Adds security headers for objects

  2. Security
    - Configures public read access for images
    - Restricts authenticated users to only upload images
...
WITH CHECK (
  bucket_id = 'images'
  AND (storage.foldername(name))[1] != 'private'
  AND (length(storage.foldername(name)) > 0)
