...
const AdminTravel: React.FC = () => {
  ...
  const [isAddingFerry, setIsAddingFerry] = useState(false);
  const [ferryFormData, setFerryFormData] = useState({
    operator: 'Sea Coach Express',
    route: 'Aberdeen Terminal ⇄ Lungi Airport',
    departure_port: 'Aberdeen Terminal',
    arrival_port: 'Lungi Airport Terminal',
    departure_time: '',
    arrival_time: '',
  ...
  const handleDeleteFerrySchedule = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this ferry schedule?')) {
      try {
        setIsLoading(true);
        const { error } = await supabase
          .from('ferry_schedules')
        ...
        console.error('Error deleting ferry schedule:', error);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleFerrySubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!ferryFormData.departure_time || !ferryFormData.arrival_time) {
      alert('Please select departure and arrival times');
      return;
    }
    try {
      setIsLoading(true);
      const { error } = await supabase.from('ferry_schedules').insert([
        {
          ...
          route: ferryFormData.route,
          departure_port: ferryFormData.departure_port,
          arrival_port: ferryFormData.arrival_port,
          departure_time: new Date(ferryFormData.departure_time).toISOString(),
          arrival_time: new Date(ferryFormData.arrival_time).toISOString(),
          booking_status: ferryFormData.booking_status,
          travel_record_id: ferryFormData.travel_record_id || null
        }
      ]);
      if (error) throw error;
      setIsAddingFerry(false);
      setFerryFormData({
        operator: 'Sea Coach Express',
        route: 'Aberdeen Terminal ⇄ Lungi Airport',
        departure_port: 'Aberdeen Terminal',
      ...
      console.error('Error adding ferry schedule:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const renderFerrySchedules = () => (
    <div className="space-y-6">
      ...
      {/* Add Ferry Booking Form */}
      {isAddingFerry && (
        <div className="bg-white rounded-lg border border-slate-200 p-6">
          <div className="flex justify-between items-center mb-4">
            <h4 className="text-lg font-bold text-slate-900">Record Delegation Ferry Booking</h4>
            <button
              type="button"
              onClick={() => setIsAddingFerry(false)}
              className="text-sm text-slate-500 hover:text-slate-700"
            >
              Cancel
            </button>
          </div>
          <form onSubmit={handleFerrySubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Operator</label>
              <select
                value={ferryFormData.operator}
              ...
              >
                <option value="Sea Coach Express">Sea Coach Express</option>
                <option value="Sea Bird Express">Sea Bird Express</option>
                <option value="Government Ferry">Government Ferry (Kissy - Tagrin)</option>
                <option value="Private Water Charter">Private Water Charter</option>
              </select>
            </div>
            <div>
              ...
              <input
                type="text"
                value={ferryFormData.route}
                onChange={(e) => setFerryFormData({ ...ferryFormData, route: e.target.value })}
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"
                placeholder="e.g. Aberdeen Terminal to Lungi Airport"
                required
              />
            </div>
            <div>
              ...
              <input
                type="text"
                value={ferryFormData.departure_port}
                onChange={(e) => setFerryFormData({ ...ferryFormData, departure_port: e.target.value })}
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"
                required
              />
            </div>
            <div>
              ...
              <input
                type="text"
                value={ferryFormData.arrival_port}
                onChange={(e) => setFerryFormData({ ...ferryFormData, arrival_port: e.target.value })}
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"
                required
              />
            </div>
            <div>
              ...
              <input
                type="datetime-local"
                value={ferryFormData.departure_time}
                onChange={(e) => setFerryFormData({ ...ferryFormData, departure_time: e.target.value })}
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"
                required
              />
            </div>
            <div>
              ...
              <input
                type="datetime-local"
                value={ferryFormData.arrival_time}
                onChange={(e) => setFerryFormData({ ...ferryFormData, arrival_time: e.target.value })}
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"
                required
              />
            </div>
            <div>
              ...
              <select
                value={ferryFormData.booking_status}
                onChange={(e) => setFerryFormData({ ...ferryFormData, booking_status: e.target.value })}
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"
              >
                <option value="confirmed">Confirmed</option>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Linked Travel Event</label>
              <select
                value={ferryFormData.travel_record_id}
                onChange={(e) => setFerryFormData({ ...ferryFormData, travel_record_id: e.target.value })}
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"
              >
                <option value="">None / Standalone</option>
                {travelRecords.map((tr) => (
                  <option key={tr.id} value={tr.id}>
                    {(tr as any).events?.title || tr.destination} ({new Date(tr.departure_date).toLocaleDateString()})
                  </option>
                ))}
              </select>
            </div>
      ...
      {/* Delegation Bookings Table */}
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div>
            <h4 className="font-semibold text-slate-900">Delegation Ferry Bookings</h4>
            <p className="text-xs text-slate-500">Transfers recorded for athletes, staff, and officials</p>
          </div>
          <span className="text-xs font-semibold px-2.5 py-1 bg-white border border-slate-200 rounded-md text-slate-700">
            {ferrySchedules.length} {ferrySchedules.length === 1 ? 'Record' : 'Records'}
          </span>
        </div>

        {ferrySchedules.length === 0 ? (
          <div className="text-center py-12 px-4">
            <Ship className="mx-auto h-10 w-10 text-slate-300 mb-2" />
            <p className="text-sm font-medium text-slate-700">No ferry transfers recorded yet</p>
            <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
              Scheduled Sea Coach or ferry transfers recorded for delegations traveling between Freetown and Lungi Airport will appear here.
            </p>
          </div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Route / Ports</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {ferrySchedules.map((schedule) => (
                  <tr key={schedule.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                      {new Date(schedule.departure_time).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}
                      {' - '}
                      {new Date(schedule.arrival_time).toLocaleTimeString([], { timeStyle: 'short' })}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                        ${schedule.booking_status === 'confirmed' ? 'bg-green-100 text-green-800' :
                          ...
                          schedule.booking_status === 'completed' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <button
                        onClick={() => handleDeleteFerrySchedule(schedule.id)}
                        className="text-red-600 hover:text-red-900"
